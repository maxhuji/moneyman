...
export class CSVExporter {
  ...
  constructor(
    ...
    this.budgetConfig = JSON.parse(fs.readFileSync(this.budgetConfigPath, 'utf-8'));
  }

  private categorizeTransaction(description: string, memo: string): string {
    const searchText = `${description} ${memo}`.toLowerCase();

    for (const [category, config] of Object.entries(this.budgetConfig.categories)) {
  ...
  private generateBudgetSummary(
    ...
    const summary = Object.entries(this.budgetConfig.categories).map(([category, config]) => ({
      category,
      budget: config.budget,
      spent: Math.round(categorySpending[category] || 0),
      remaining: Math.round(config.budget - (categorySpending[category] || 0)),
      percentUsed: Math.round(((categorySpending[category] || 0) / config.budget) * 100),
    }));
