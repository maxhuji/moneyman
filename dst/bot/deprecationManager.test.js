...
describe("deprecationManager", () => {
    let sendDeprecationMessage;
    let assignDeprecationHandler;
    beforeEach(async () => {
        jest.resetModules();
    ...
    describe("sendDeprecationMessage", () => {
        it("should call handler immediately when handler is assigned", () => {
            ...
            expect(mockHandler).toHaveBeenCalledWith("hashFiledChange", expect.stringContaining("⚠️ Deprecation warning:"));
        });
        ...
        ])("message formatting for $messageId", ({ messageId, expectedContent }) => {
            it.each(expectedContent)("should include '%s' in the message", (content) => {
                const mockHandler = jest.fn();
                assignDeprecationHandler(mockHandler);
                sendDeprecationMessage(messageId);
                expect(mockHandler).toHaveBeenCalledWith(messageId, expect.stringContaining(content));
            });
        });
    });
    describe("assignDeprecationHandler", () => {
        ...
        it("should format messages correctly when calling handler", () => {
            ...
            sendDeprecationMessage("hashFiledChange");
            assignDeprecationHandler(mockHandler);
            expect(mockHandler).toHaveBeenCalledWith("hashFiledChange", expect.stringMatching(/⚠️ Deprecation warning:\s*This run is using the old transaction hash field/));
        });
    });
});
//# sourceMappingURL=deprecationManager.test.js.map