...
export async function saveResults(results) {
    ...
    const context = {
        accountResults: results.map((r) => ({
            ...
            txnCount: r.result.accounts?.reduce((sum, a) => sum + a.txns.length, 0) ?? 0,
        })),
    };
    await parallel(storages.map((storage) => async () => {
        ...
        return loggerContextStore.run({ prefix: `[${name}]` }, async () => {
            try {
                ...
                steps.at(-1)?.end();
                logger(`saved`);
                await editMessage(message?.message_id, statsString(stats, duration, steps));
            }
            catch (e) {
...
function resultsToTransactions(results) {
    const txns = [];
            ...
            for (let account of result.accounts ?? []) {
                for (let tx of account.txns) {
                    try {
                        ...
                        sendError(error, `Failed to process transaction for ${companyId} account ${account.accountNumber}:\n${JSON.stringify(tx, null, 2)}`);
                    }
                }
            }
        }
