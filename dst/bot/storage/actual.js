...
export class ActualBudgetStorage {
    config;
    bankToActualAccountMap = new Map();
    ...
    async saveTransactions(txns, onProgress) {
        ...
        const stats = createSaveStats("ActualBudgetStorage", `budget: "${this.config.storage.actual?.budgetId || "unknown"}"`, txns);
        try {
            ...
            for (let tx of txns) {
                ...
                const actualTx = this.convertTransactionToActualFormat(tx, actualAccountId);
            ...
            if (transactionsByActualAccountId.size > 0) {
                await this.sendTransactionsToActual(transactionsByActualAccountId, stats, onProgress);
            }
        }
        finally {
    ...
    async sendTransactionsToActual(transactionsByActualAccountId, stats, onProgress) {
        logger(`sending to Actual budget: "${this.config.storage.actual?.budgetId}"`);
        try {
            for (const [actualAccountId, transactions,] of transactionsByActualAccountId) {
                const accountName = this.accountIdToNameMap.get(actualAccountId) || actualAccountId;
                logger(`Processing ${transactions.length} transactions for account "${accountName}"`);
                ...
                if (importResponse.errors?.length) {
                    logger(`Errors importing transactions: ${JSON.stringify(importResponse.errors)}`);
                    continue;
                }
                logger(`Imported ${importResponse.added?.length || 0} transactions for account "${accountName}"`);
    ...
    async init() {
        ...
        try {
            ...
            this.accountIdToNameMap = new Map(actualAccounts.map((a) => [a.id, a.name]));
            this.bankToActualAccountMap = new Map(Object.entries(actualConfig.accounts));
            for (const [bankAccountId, actualAccountId,] of this.bankToActualAccountMap.entries()) {
                if (!validActualAccountIds.has(actualAccountId)) {
                    const accountName = this.accountIdToNameMap.get(actualAccountId) || actualAccountId;
                    logger(`Warning: Actual Budget account "${accountName}" for bank account ${bankAccountId} does not exist`);
                    this.bankToActualAccountMap.delete(bankAccountId);
                }
            }
            if (this.bankToActualAccountMap.size === 0) {
                throw new Error("No valid account mappings found. Please check ACTUAL_ACCOUNTS configuration.");
            }
        }
        catch (error) {
            throw new Error(`Failed to initialize Actual Budget: ${error instanceof Error ? error.message : error}`);
        }
    }
    convertTransactionToActualFormat(tx, actualAccountId) {
        ...
        return {
            ...
            imported_id: hash(this.config.options.scraping.transactionHashType === "moneyman"
                ? tx.uniqueId
                : tx.hash).toString(),
            notes: tx.memo,
        };
