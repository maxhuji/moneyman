...
import { sendAndDeleteLogFile, enableDebugLoggingIfNeeded, } from "./utils/secure-log.js";
...
process.exit(0);
async function runScraper(hooks) {
    try {
        logger("About to start");
        await hooks.onBeforeStart();
        logger("Starting to scrape");
        const results = await scrapeAccounts(scraperConfig, async (status, totalTime) => {
